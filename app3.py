...
def generate_custom_truss(params):
    ...
    excel_bytes = save_formatted_excel_bytes(raw_data)
    
    return fig, pdf_buffer, excel_bytes, f"{file_prefix}.pdf", f"{file_prefix}_재단표.xlsx"
...
def run_ladder_system(params):
    ...
    excel_bytes = save_ladder_excel_bytes(data, L_cm)
    ...
    with PdfPages(pdf_buffer) as pdf:
        ...
        for i in range(n_sec_m + 1):
            ...
            if i < n_sec_m:
                cx_start = i * gap_m
                if n_sec_m == 1:
                    x_min, x_max, label = ridge_deduct_cm, L_cm - ridge_deduct_cm, "[단일세트]"
                elif i == 0:
                    x_min, x_max, label = ridge_deduct_cm, gap_m - ridge_deduct_cm / 2, "[양끝세트]"
                elif i == n_sec_m - 1:
                    x_min, x_max, label = cx_start + ridge_deduct_cm / 2, L_cm - ridge_deduct_cm, "[양끝세트]"
                else:
                    x_min, x_max, label = cx_start + ridge_deduct_cm / 2, (i+1)*gap_m - ridge_deduct_cm / 2, "[중간세트]"
    ...
    pdf_buffer.seek(0)
    return fig, pdf_buffer, excel_bytes, pdf_file_name, f"종합산출표_{int(L_cm)}cm.xlsx"
...
with tab1:
    ...
    with col1:
        ...
        h_tie_cm = 0.0
        if has_tie: h_tie_cm = st.number_input("수평재(수평선) 바닥기준 높이(cm)", value=150.0)
